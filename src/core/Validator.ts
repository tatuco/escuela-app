import {getTypeNumber, validateArrayObjects} from "./Utils";
import {User} from "../entity/User";


export class Validator {
    static make(request, validations): any {
        try {
                    ...
                    if (objectBody.hasOwnProperty(it)
                        ...
                        if (typeof objectBody[it] !== type && typeof objectBody[it] !== 'object') {
                            if ((type === "integer" || type === "decimal") && typeof objectBody[it] === 'number') {
                                if (getTypeNumber(objectBody[it]) !== type) {
                        ...
                        if (type === "array" || type.indexOf("=>") !== -1) {
                            ...
                            if (validationsArray[i].indexOf("=>") !== -1) {
                                const validate = validateArrayObjects(validationsArray[i].split("=>")[1],objectBody[it], it)
